...
class AnimationProcessorWrapper:
	...
	def run_thread(self, ip, plugin, palette, _palette_entered, use_simulator, use_plugin_options):
		if self.is_running():
			self.stop()
		time.sleep(1)
		...
		self.use_plugin_options = use_plugin_options
		...
		iprint("Animation processor thread launched")

	def run(self):
		iprint("Starting up animation processor")
		...
		if self.palette_entered:
			command = ["./AnimationProcessor", "-i", str(self.ip_addr), "-p", lib_path, "-cp", self.palette_path]
		else:
			command = ["./AnimationProcessor", "-i", str(self.ip_addr), "-p", lib_path]
		if self.use_simulator:
			command.append("-s")
		if self.use_plugin_options:
			plugin_options_path = self.plugin_path + self.directory_divider + "inc" + self.directory_divider + "PluginOptions.h"
			command.append("-plugin_opt")
			command.append(plugin_options_path)
		command.append("1>$2")
		...
		try:
			...
			if (str(oserr.errno) == "2"):
				...
				iprint("Please make sure you have AnimationProcessor")
			else:
				iprint(oserr)
			return
		while not self.abort_signal:
			...
			time.sleep(1)
		iprint("Exiting animation processor thread")
		self.abort_signal = False
	...
	def stop_music_proc(self):
		...
		if self.mp_proc != None:
			try:
				self.mp_proc.stdin.write('q'.encode())
			except IOError:
				pass
			self.mp_proc.kill()
			self.mp_proc = None
